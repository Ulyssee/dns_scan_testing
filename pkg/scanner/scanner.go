	...
	return func(s *Scanner) error {
		...
		for i := 0; i < len(ns); i++ {
			if host, port, err := net.SplitHostPort(ns[i]); err != nil {
				...
				ns[i] = net.JoinHostPort(host, "5000")
			}
		}
		s.Nameservers = ns[:]
		return nil
...
func (s *Scanner) Scan(name string) *ScanResult {
	...
	print(name, "name of domain \n")
	print(res, "res of domain \n")

	switch s.RecordType {
	case "all":
		print(s.RecordType, "record type \n")
		if err = s.GetDNSRecords(res, "DKIM", "DMARC", "SPF", "BIMI"); err != nil {
...
func (s *Scanner) GetNS() string {
	print(s.Nameservers, "nameserver\n")
